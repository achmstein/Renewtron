...
public class DataGovBusinessNameService : IBusinessNameFallbackService
{
    ...
    public DataGovBusinessNameService(HttpClient httpClient, ILogger<DataGovBusinessNameService> logger)
    {
        ...
        // data.gov.au's CKAN datatables endpoint blocks bare HttpClient requests with a 403.
        ...
        if (!_httpClient.DefaultRequestHeaders.UserAgent.Any())
        {
            ...
            _httpClient.DefaultRequestHeaders.AcceptLanguage.ParseAdd("en-AU,en;q=0.9");
            _httpClient.DefaultRequestHeaders.Add("X-Requested-With", "XMLHttpRequest");
            _httpClient.DefaultRequestHeaders.Referrer = new Uri($"https://data.gov.au/data/dataset/asic-business-names/resource/{ResourceId}");
        }
    }
    ...
    public async Task<BusinessNamesResult> SearchByAbnAsync(string abn)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning(
                    "data.gov.au returned {StatusCode} for ABN {Abn}. Body: {Body}",
                    (int)response.StatusCode, cleanAbn, body.Length > 500 ? body[..500] : body);
                return BusinessNamesResult.Failed($"data.gov.au returned HTTP {(int)response.StatusCode}");
            }
